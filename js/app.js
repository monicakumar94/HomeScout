...
// Simple search action (route to your search page when ready)
const searchBtn = document.getElementById("searchBtn");
const searchInput = document.getElementById("searchInput");
if (searchBtn && searchInput) {
  searchBtn.addEventListener("click", () => {
    const q = (searchInput.value || "").trim();
    // TODO: wire to your own results page
...
const demoHomes = [
  {
    price: 12888800,  // INR example; change to your data
...
if (track) {
  ...
  const shift = () => Math.min(rail.clientWidth, 340); // move by one card

  prev?.addEventListener("click", () => rail.scrollBy({ left: -shift(), behavior: "smooth" }));
  next?.addEventListener("click", () => rail.scrollBy({ left:  shift(), behavior: "smooth" }));
}
